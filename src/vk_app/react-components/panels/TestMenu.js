...
export default function TestMenu(props) {
  ...
  return (
    <Panel id={ props.id }>
      ...
      <FormLayout>
        <FormLayoutGroup top={`Язык для проверки знаний`}>
          <Radio name="subject1" value="geom" Checked={ state.types.radio }>Английский язык</Radio>
          <Radio name="subject2" value="Jap" Checked={ state.types.radio }>Японский язык</Radio>
          <Radio name="subject3" value="Ger" Checked={ state.types.radio }>Немецкий язык</Radio>
          <Radio name="subject4" value="Ital" Checked={ state.types.radio }>Итальянский язык</Radio>
        </FormLayoutGroup>
      </FormLayout>
